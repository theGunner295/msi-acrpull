...
FROM mcr.microsoft.com/oss/go/microsoft/golang:1.27-azurelinux3.0 AS builder
ARG TARGETOS
ARG TARGETARCH
...
RUN CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -a -o manager -tags=requirefips cmd/main.go
